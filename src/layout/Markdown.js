...
function Markdown() {
    ...
    // Inline svg used to simplify changing colors
    const setPreviewEye = () => {
        let previewEye;
        if(preview) {
            previewEye = closedEyeIcon;
        } else {
            previewEye = eyeIcon;
        }
        return previewEye;
    } // Changes eye icon to closed or open based on preview state
  return (
    <>
        <div className={`container ${!minWidth768 && 'mobile' }`}>
            <div className={`markdown ${theme} ${preview && 'invis'}`}>
                ...
                <textarea className="textArea codeText"
                onChange={e => setMarkdown(e.target.value)}
                ></textarea>
            </div>
            

            <div className={`output ${theme} ${!preview && !minWidth768 && 'invis'}`}>            
                <Header name='PREVIEW' onClick={onClick} image={setPreviewEye()} />

                <div className={`preview ${preview && 'fullPreview'}`}>
                    <ReactMarkdown>
                        {markdown}
                    </ReactMarkdown>
                </div>
